...
export class FormUtils {
  ...
  static toggleOneRequired(formGroup: FormGroup, formControlName1: string, formControlName2: string) {
    ...
    if (CommonUtils.isVoid(control1)) {
      LoggerService.logWarn(this.constructor.name, 'checkOnlyOneRequired',
        `Attenzione!: il formControl: ${formControlName1} non è presente nel formGroup: ${formGroup}`);
    }
    if (CommonUtils.isVoid(control2)) {
      LoggerService.logWarn(this.constructor.name, 'checkOnlyOneRequired',
        `Attenzione!: il formControl: ${formControlName2} non è presente nel formGroup: ${formGroup}`);
    }
  ...
  static toggleValidator(formGroup: FormGroup, formControlName: string, validator: ValidatorFn, condition: boolean) {
    ...
    if (CommonUtils.isVoid(control)) {
      LoggerService.logWarn(FormUtils.name, 'toggleValidator',
        `Attenzione!: il formControl: ${formControlName} non è presente nel formGroup: ${formGroup}`);
    }
  ...
  static toggleError(formGroup: FormGroup, formControlName: string, isValid: boolean, errorCode: string) {
    ...
    if (CommonUtils.isVoid(control)) {
      LoggerService.logWarn(FormUtils.name, 'toggleError',
        `Attenzione!: il formControl: ${formControlName} non è presente nel formGroup: ${formGroup}`);
    }
      ...
      Object.keys(form.controls).forEach(key => {
        ...
        if (controlErrors != null) {
          Object.keys(controlErrors).forEach(keyError => {
            LoggerService.logWarn(caller, 'getFormValidationErrors', 'KEY: ' + key + ', ERROR: ' + keyError);
          });
        }
      });
    }
